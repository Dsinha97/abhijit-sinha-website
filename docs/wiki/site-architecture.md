...
3. **`<slot/>`** — page content.
4. **Footer** — statutory risk warning box, distributor disclaimer, CAMS/KFintech/SEBI SCORES 2.0 links, office/registration metadata, copyright. Since 2026-09-06 the identity block carries the **full office address** rather than the bare city, and the contact links are a wrapping flex row of email · phone · "View on Google" — see [contact-channels](contact-channels.md). The two sit **stacked at every breakpoint**, not side by side: the block was `flex-col sm:flex-row … justify-between`, which was fine while the identity line ended in "Navi Mumbai" and cramped once it ended in a full postal address. Copyright, ARN and office address now read as three `|`-separated fields, the separators being `aria-hidden` spans rather than bare pipe characters, so a screen reader does not announce them. Full text in [regulatory-compliance](regulatory-compliance.md).
