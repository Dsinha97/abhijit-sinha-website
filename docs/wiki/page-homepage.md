...
1. **Hero banner** — `banner.png` (≥768px) / `banner-mobile.jpeg` (<768px), shown whole and undistorted (`aspect-ratio` per breakpoint, no crop, no dark overlay). The banner graphics are *finished designs* that already bake in the logo, headline, tagline, and all four pillars (Goal-Based Investing, Risk-Managed Portfolios, Disciplined Wealth Creation, Long-Term Financial Growth) — don't re-render that copy as an overlay on top of it; carry it instead in the image's `alt` text for accessibility/SEO.
   ...
   The banner is 2:1, so at typical desktop widths it renders taller than the viewport. This is the one page that sets `overlayHeader` on `BaseLayout` — the regulatory strip and nav go translucent and run all the way to the top of the page instead of sitting above it (see [site-architecture](site-architecture.md#overlay-header-homepage-only)), and the CTA band (the page's one `<h1>`, a supporting line, and the two buttons: "Schedule a 15-Minute Intro Call" → `/schedule`, "Explore Investment Solutions" → `/solutions`) is `sticky bottom-0` as the hero's last child. That pins it to the bottom of the window while the banner scrolls past above it — so the actions are visible without scrolling on first load — and releases into normal flow the instant the hero section ends, with no hardcoded height/margin math.
...
3. **About Abhijit Sinha** (`id="about"`) — two columns:
   - Left: `profile-pic.jpg` (`object-top` framing so the face isn't cropped), name, credentials box (Commerce Graduate, MBA Finance, 35 years AM industry experience, former Head of Sales Management at a leading Indian AMC, ARN-367596), then "Schedule a Conversation" (`/schedule`) and "Connect on LinkedIn" (`distributor.linkedin`, new tab) stacked underneath.
   - Right: first-person bio (four paragraphs — career history, sales-management background, distributor mission, investing philosophy).
