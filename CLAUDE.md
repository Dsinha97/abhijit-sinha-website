...
- Never change these identifiers without the user explicitly providing a new value: **ARN-367596**, **EUIN E703717**, **NISM Series V-A**, principal place of business **Sector 35E, Kharghar, Navi Mumbai, Panvel, Maharashtra 410210**, phone **+91-8976539234**, email **support@abhijitsinha.in**.
- The address and phone above are also a **NAP pair with the Google Business Profile** (place ID `ChIJ9zAUWBnB5zsRUORqF5Cp7OA`, in `googleBusiness` in `site.ts`) and are emitted as `PostalAddress` + `sameAs` in the JSON-LD. Editing either in `site.ts` without editing the listing to match breaks the local-search link between them. Google links are plain anchors, never embeds — a map iframe would need `frame-src https://www.google.com` in `vercel.json` and would break the "nothing loads before you click" promise the site keeps for YouTube and Calendly. See [docs/wiki/contact-channels.md](docs/wiki/contact-channels.md).
...
- `supabase/functions/` holds the source for `track`, `verify-lead` and `publish-site` (never `submit-lead`, whose source is not in this repo). Deploy new ones with `--no-verify-jwt`. All three share a `VERCEL_PREVIEW` regex anchored to this project's preview hostnames — **do not loosen it back to a bare `*.vercel.app` wildcard**, which would let anyone's Vercel deployment pass the origin check; it is the last barrier on `verify-lead` whenever `TURNSTILE_SECRET_KEY` is unset.
- `supabase/schema.sql` is a committed snapshot of the live schema — tables, RLS policies, functions, triggers, cron — for review and drift detection. It is **not a migration and must never be executed**; it lives outside `supabase/migrations/` precisely so `supabase db push` cannot pick it up. The live database stays the source of truth; refresh the file when the schema changes. `supabase/` is excluded from `tsconfig.json`: those are Deno files, and `npm run build` runs `astro check` first, so leaving them in would fail every site deploy on phantom type errors.
...
Push to `main` triggers a Vercel build and deploy (`npm run build` → `dist/`). There is no GitHub Actions workflow and no GitHub Pages source — do not reintroduce either; a second live copy of a regulated-content site is a compliance problem, not just an SEO one. Code scanning therefore uses **CodeQL default setup**, which runs from repo settings with no workflow file committed; Actions is restricted to GitHub-owned actions so that setup can run and nothing else can.

**`main` is protected: every change needs a pull request.** Zero approvals are required (a solo maintainer cannot approve their own PR) and admin bypass is on for emergencies, but the normal path is branch → PR → merge, not a direct push. Force-pushes and branch deletion are blocked. Dependabot security updates are enabled, so security-fix PRs will appear on their own.
